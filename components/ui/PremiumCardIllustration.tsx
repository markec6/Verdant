...
function StaticIllustrationArt({
  colors,
  variant,
}: {
  ...
  switch (variant) {
    ...
    case "assessment":
      return (
        <svg aria-hidden="true" className="premium-card-art__svg" viewBox="0 0 64 64">
          <path d="M14 46h36" stroke={colors.muted} strokeDasharray="3 4" strokeWidth="1.8" />
          <path d="M22 38l24-18" stroke={colors.primary} strokeLinecap="round" strokeWidth="2.5" />
          <rect
            fill={colors.soft}
            height="10"
            rx="1.5"
...
export default function PremiumCardIllustration({
  ...
  const iconClassName = `card-expand-reveal__icon ${
  ...
  if (isMobile) {
    return (
      <div className={iconClassName}>
        <StaticIllustrationArt colors={colors} variant={variant} />
      </div>
    );
  }
